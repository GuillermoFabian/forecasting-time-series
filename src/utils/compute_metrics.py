...
def compute_eval_data_ratio(training_df, evaluation_df):
    ...
    """
    # calculate mean and standard deviation of sales by time series for training and eval data each
    training_stats_df = training_df.groupby('series_id').agg(mean_train_sales=('sales', np.mean), stdev_train_sales=('sales', np.std))
    eval_stats_df = evaluation_df.groupby('series_id').agg(mean_eval_sales=('sales', np.mean), stdev_eval_sales=('sales', np.std))
